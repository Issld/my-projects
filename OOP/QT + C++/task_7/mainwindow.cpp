    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(&info, &States::notifyObservers, this, &MainWindow::update); //Отлов сигнала из класса states
...
void MainWindow::update()
{
    ...
    ui->Previous->setEnabled(info.myHasStates()); //Проверка нужно ли гасить кнопку
    if (!info.myHasStates()){ //Чистка массива, если в нем менее 2-х элементов
        info.Clear();
    }
    obj = nullptr;
}
...
{
    Estate* obj = processForm(); //создание объекта
    showCost(obj);
    info.add(obj); //добавление в массив состояний
...
{
    ui->Output->setText(QString::number(CalculationFacade::getCost(obj))); //Рассчет страховой стоимости через паттерн "Фасад"
}
